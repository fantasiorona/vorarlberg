...
# papers
- DESIGN AND IMPLEMENTATION OF A PARALLEL GENETIC ALGORITHM FOR THE TRAVELLING PURCHASER PROBLEM - Luiz S. Ochi, Lficia M. A. Drummond
- The Generalized Traveling Salesman Problem: A New Genetic Algorithm Approach - John Silberholz and Bruce L. Golden
- A NEW GENETIC ALGORITHM APPLIED TO THE TRAVELING SALESMAN PROBLEM - Sawsan K. Amous, Taicir Loukil, Semya Elaoud, Clarisse Dhaenens
- The Traveling Purchaser Problem and its variants - Daniele Manerbaa, Renata Mansini, Jorge Riera-Ledesma
- Transgenetic algorithm for the Traveling Purchaser Problem- M.C. Goldbarg, L.B. Bagi, E.F.G. Goldbarg

# problems:
- crossover: how to handle different array sizes?
    --> building clusters which connect all purchasable goods, so the size is always = provided goods
...
int main (int argc, char * argv[])
{
    ...
    {
        if (strcmp(argv[i], "-f") == 0) dataPath = argv[++i];
        ...
        else goods.insert(std::stoi(argv[i])); // remaining given values are interpreted as goods
        // available goods for romaniaroads are (7): 4, 5, 6, 7, 8, 9, 13
    }

    if (goods.size() < 2)
    {
        std::cout << "not enough goods provided to justify some work: " << goods.size() << " min: 2" << std::endl;
        return 0;
    }
    ...
    {
        std::cout << "  " << idx++ << ": " << city.name << " (good: " << city.good << ") with roads (" << city.roads.size() << "): " << std::endl;
    ...
    if (cluster.size() < 2)
    {
        std::cout << "not enough valid goods provided to justify some work: " << cluster.size() << " min: 2" << std::endl;
        return 0;
    }
    ...
    for (const auto &entry : cluster)
    {
        ...
        {
            std::cout << "\t" << node << " - " << cities[node].name << " (" << cities[node].good << ")" << std::endl;
        }
    }
#endif

    // calculate distance for cluster nodes (could be done hardcoded because we only use the romaniaroads data file)
    std::map<int, int> nodeDistances; // key is bitwise combination of node indices, only storing one direction (smaller index to bigger)
    // omg noooe what am i doin? oO
    // TODO: double iterate cities, starting inner loop at current outer loop index, using only relevant nodes and discarding same goods
    for (const auto &setFrom : cluster)
    {
        for (const auto &setTo : cluster)
        {
            if (setFrom.first == setTo.first) continue; // no connections needed between cities with the same goods
...
#ifdef VERBOSE
    std::cout << "calculated distances (" << nodeDistances.size() << "):" << std::endl; // should be "factorial for sums" - cities with the same good
    for (const auto &dist : nodeDistances)
    {
        std::cout << "  " << (dist.first >> KEY_UPPER) << " - " << (dist.first & KEY_LOWER) << " = " << dist.first << ": "
                  << cities[dist.first >> KEY_UPPER].name << " - " << cities[dist.first & KEY_LOWER].name << ": " << dist.second << std::endl;
    }
#endif

    // 3. generate initial purchase order
    // --------------------------------------------------------------------------------------------------------------------------------------------------------
    //std::vector<std::unordered_set<size_t>> population; // should use a set for intentional reasoning, but then all the shuffle algorithms would not work :/
    ...
    for (size_t i = 0; i < populationSize; ++i)
    {
        ...
        std::random_shuffle(genome.begin() + (-1 != startIdx), genome.end()); // don't shuffle starting point if given
